      ...
      style={
        {
          ...
          "--width": `${width}`,
          "--aspect-ratio": `${ar}`,
          "--background": `linear-gradient(hsla(${hue}, 100%, ${lightness}%, 0.3), transparent)`,
...
export const WarpBackground: React.FC<WarpBackgroundProps> = ({
  ...
  beamDelayMin = 0,
  beamDuration = 3,
  gridColor = "rgba(255, 255, 255, 0.05)", // Updated default gridColor
  ...props
}) => {
  const generateBeams = useCallback(() => {
    ...
    for (let i = 0; i < beamsPerSide; i++) {
      const beam = predefinedBeams[i % predefinedBeams.length];
  ...
  return (
    <div className={cn("relative rounded border border-gray-700 p-20 bg-gray-900", className)} {...props}>
      <div
        style={
          {
